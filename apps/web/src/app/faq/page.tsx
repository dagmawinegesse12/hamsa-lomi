    ...
    <PublicShell lang={lang}>
      ...
      <section className="mx-auto max-w-3xl px-4 sm:px-6 py-12 sm:py-14">
        <dl className="space-y-4">
          {site.faq.map((item, i) => (
            <div key={i} className="card-accent border-l-gold-500 p-5 sm:p-6">
        ...
        <div className="mt-10 rounded-xl bg-green-700 p-6 text-white text-center">
          <p className="font-display text-xl font-bold mb-2">{t.stillHave}</p>
          <p className="text-sm leading-relaxed text-green-100 mb-5">{t.reachUs}</p>
          <div className="flex flex-wrap justify-center gap-3">
            <a href={`mailto:${publicSite.contact.email}`} className="rounded-md bg-gold-500 px-5 py-2.5 text-sm font-bold text-ink hover:bg-gold-400 transition-colors">
              {t.emailUs}
            </a>
            <Link href={lang === "am" ? "/contact?lang=am" : "/contact"} className="rounded-md border border-green-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-green-600 transition-colors">
              {t.contactPage}
            </Link>
          </div>
        </div>
