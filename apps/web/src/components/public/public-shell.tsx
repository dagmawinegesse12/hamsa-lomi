...
export function PublicShell({ children, lang = "en" }: { children: React.ReactNode; lang?: Lang }) {
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
          ...
          <nav className="hidden lg:flex items-center gap-1" aria-label="Main navigation">
            {publicNavItems.map((item) => (
              <NavLink
                key={item.href}
                href={lang === "en" ? item.href : `${item.href}?lang=am`}
                exact
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setOpen(false)} aria-label="Close menu" className="flex h-9 w-9 items-center justify-center rounded-md text-ink-muted hover:bg-gray-100 transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="border-t border-gray-100 p-4 space-y-2">
              <Link href="/apply" onClick={() => setOpen(false)} className="flex w-full items-center justify-center rounded-md bg-green-700 px-4 py-3 text-sm font-semibold text-white hover:bg-green-800 transition-colors">
                {t.nav.applyFull}
              </Link>
              <Link href="/sign-in" onClick={() => setOpen(false)} className="flex w-full items-center justify-center rounded-md border border-gray-300 px-4 py-3 text-sm font-semibold text-ink-soft hover:bg-gray-50 transition-colors">
                {t.nav.memberLogin}
              </Link>
            </div>
      ...
      <footer className="bg-ink text-cream-200 mt-16">
          ...
          <div className="grid gap-10 md:grid-cols-[1.2fr_1fr_1fr_1fr]">
            <div>
              ...
              <p className="text-sm leading-relaxed text-cream-400 max-w-xs">
                {lang === "am" ? "የNashvilleን የኢትዮጵያ ማህበረሰብ በባህል፣ በርህራሄ እና በጋራ ኃላፊነት መርዳት።" : publicSite.tagline}
              </p>
              <div className="mt-4 rounded-md border border-green-700/40 bg-green-700/20 p-3">
                <p className="font-display text-sm text-cream-200 leading-snug">{publicSite.mottoAmharic}</p>
                <p className="mt-1 text-xs italic text-cream-400">{publicSite.mottoEnglish}</p>
              </div>
            </div>

            <div>
              <p className="mb-3 text-xs font-bold uppercase tracking-widest text-gold-500">{t.footer.org}</p>
              <ul className="space-y-2 text-sm">
                {publicNavItems.map((l) => (
                  <li key={l.href}>
                    <Link href={lang === "en" ? l.href : `${l.href}?lang=am`} className="text-cream-400 hover:text-gold-400 transition-colors">
                      {lang === "am" ? l.labelAm : l.label}
                    </Link>
                  </li>
                ))}
              </ul>
            ...
            <div>
              <p className="mb-3 text-xs font-bold uppercase tracking-widest text-gold-500">{t.footer.resources}</p>
              <ul className="space-y-2">
                {publicSite.helpfulLinks.flatMap((s) =>
                  s.items.map((item) => (
                    <li key={item.url}>
                      <a href={item.url} rel="noreferrer" target="_blank" className="text-sm text-cream-400 hover:text-gold-400 transition-colors">
                        {item.name}
                      </a>
                    </li>
                  ))
            ...
            <div>
              <p className="mb-3 text-xs font-bold uppercase tracking-widest text-gold-500">{t.footer.contact}</p>
              <address className="not-italic space-y-2 text-sm text-cream-400">
                ...
                <a href={`mailto:${publicSite.contact.email}`} className="block text-gold-400 hover:text-gold-300 transition-colors mt-3">{publicSite.contact.email}</a>
                <a href={`tel:+1${publicSite.contact.phone.replace(/\D/g, "")}`} className="block text-cream-400 hover:text-cream-200 transition-colors">{publicSite.contact.phone}</a>
              </address>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-ink-soft/30">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-4 text-xs text-cream-500">
            <p>© {new Date().getFullYear()} Hamsa Lomi Ethiopian Association. {t.footer.rights}</p>
            <div className="flex gap-4">
              <Link href={lang === "en" ? "/contact" : "/contact?lang=am"} className="hover:text-cream-300 transition-colors">{t.footer.contact}</Link>
              <Link href={lang === "en" ? "/bylaws" : "/bylaws?lang=am"} className="hover:text-cream-300 transition-colors">{lang === "am" ? "ደንብና መመሪያ" : "Bylaws"}</Link>
              <Link href="/sign-in" className="hover:text-cream-300 transition-colors">{t.footer.memberPortal}</Link>
            </div>
          </div>
        </div>
      </footer>
